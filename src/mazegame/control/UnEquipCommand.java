		...
		if (thePlayer.getInventory().getItemList().containsKey(itemName)) {
			...
			if (item.isEquiped()) {
				...
				if (item instanceof Armor) {
					...
					message.append("The player has successfully took off the " + item.getValue() + " armour.\n");
				} else if (item instanceof Weapon) {
					thePlayer.setStrength(thePlayer.getStrength() - ((Weapon) item).getCombatPower());
					message.append("The player has successfully unweild the " + item.getValue() + " weapon.\n");
				} else if (item instanceof Shield) {
					thePlayer.setLifePoints(thePlayer.getLifePoints() - ((Shield) item).getProtection());
					message.append("The player has successfully unwield the " + item.getValue() + " shield.\n");
				}
				message.append(thePlayer.getCurrentPlayerStat(thePlayer));

				item.setEquiped(false);
			} else {
				message.append("You are not wearing/holding the " + item.getValue() + "\n");
